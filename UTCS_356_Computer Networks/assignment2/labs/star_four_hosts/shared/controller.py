    ...
    try:
        ...
        while (True):
            ...
            if (digest_type == 'digest'):
                ...
                smac_entry = p4info_helper.buildTableEntry(table_name="MyIngress.smac_table", match_fields={"hdr.ethernet.source": eth_src_addr}, action_name="NoAction")
                smac_entry.idle_timeout_ns = int(TIMEOUT_SEC * 1e9)
                s1.WriteTableEntry(smac_entry)

                dmac_entry = p4info_helper.buildTableEntry(table_name="MyIngress.dmac_forward", match_fields={"hdr.ethernet.destination": eth_src_addr}, action_name="MyIngress.forward_to_port", action_params={"egress_port": port_id})
                dmac_entry.idle_timeout_ns = int(TIMEOUT_SEC * 1e9)
