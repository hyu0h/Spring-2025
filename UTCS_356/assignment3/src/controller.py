        ...
        while (True):
            ...
            if (msg.HasField('packet')):
                ...
                if (op == CONTROLLER_OP_ARP_ENQUEUE):
                    ...
                    s1.WriteTableEntry(table_entry)
                    print("Added ARP table entry for", next_hop_ip, next_hop_mac)

                    # Add DMAC forwarding table entry: when destination MAC equals next_hop MAC,
                    # forward packet to the appropriate port using the egress MAC.
                    table_entry = p4info_helper.buildTableEntry(
                    table_name="MyIngress.dmac_forward",
                    match_fields={"hdr.ethernet.dstAddr": next_hop_mac},
                    action_name="MyIngress.forward_to_port",
                    ...
                    s1.WriteTableEntry(table_entry)
                    print("Added DMAC forwarding table entry for", next_hop_mac, egress_port)
                        ...
                        for i in range(numEntry):
                            ...
                            if (entry.addr in routing_table):
                                ...
                                if routing_table[entry.addr].mergeRoute(newRoute):
                                    table_entry = p4info_helper.buildTableEntry(table_name="MyIngress.ipv4_route", match_fields={"hdr.ipv4.dstAddr": (entry.addr, 32)}, action_name="MyIngress.forward_to_next_hop", action_params={"next_hop": str(newRoute.nextHopIP)})
